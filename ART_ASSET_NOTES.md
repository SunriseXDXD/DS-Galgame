...
2026-09-08 使用内置 imagegen 的 `identity-preserve` 编辑模式，以已入库的 `dafeiyu-v2-neutral.webp` 为唯一身份、服装和布局参考，分别生成 `bashful`、`cheer`、`explain`、`point` 四个独立动作。没有提交额外第三方角色图，也没有调用 CLI/API 备用生成路径。

项目内原稿和完整实际提示词保存在 [artwork/action-drafts/PROMPTS.md](artwork/action-drafts/PROMPTS.md) 及同目录四张 PNG。动作分别是双手贴近面颊侧目、单拳欢呼、摊手讲解和朝画面左侧指点；保留成年角色、长裙、头饰、尾鳍与脚底布局。

原稿均为 `1024×1536` 不透明 RGB PNG，背景棋盘格被烘焙进像素。对 `point` 额外尝试了一次内置背景提取编辑，但结果仍是不透明渐变底，故未采用。原稿尚未作为新透明立绘接入，运行时动作映射暂回退到旧情绪资产；透明成品验收前，不把动作指令数量声称为新增可用立绘数量。
